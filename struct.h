...
{
	if (query_section == NULL || domain_name == NULL)
		return -1;
...
void CreateRR(struct DNS_RR *RR, char *name, unsigned short type, unsigned short _class, unsigned int ttl, unsigned short pre, char *rdata)
{
	...
	RR->ttl = ttl; // data_len
	if (type == TYPE_A)
		RR->data_len = 4; // 对于IP，长度为4 data_len是编码后的长度，length是非编码长度，注意
	else
		RR->data_len = strlen(rdata) + 2; // 对于域名，生成data_len包含末尾结束符（域名末尾结束符）
...
{
	if (header_section == NULL || query_section == NULL || request == NULL)
		return -1;
