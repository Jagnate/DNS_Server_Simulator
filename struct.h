                                ...
                                unsigned short rcode)
{
	...
	header_section->tag = htons(CreateTag(qr,opcode,aa,tc,rd,ra,z,rcode)); //标准查询
	header_section->queryNum = htons(0x0001);   //只查1个结果
    header_section->answerNum = htons(0); 
    header_section->authorNum = htons(0); 
    header_section->addNum = htons(0); 
